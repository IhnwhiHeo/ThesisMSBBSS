...
%The current study focuses on finding the incorrectly labeled cases, instead of predicting the correct label for each case. If the erroneous cases can be pointed out, these can be corrected with manual editing. The website texts are used to obtain a prediction for the economic activity of an enterprise. For every enterprise there is also an observed economic activity class available in the GBR. This observed label 
...
%% use different starting values: estimate tau_i for ALL cases in the data!
